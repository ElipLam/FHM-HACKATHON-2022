...
PLAYER_WL_PATH = str(Path(ROOT, "crawled_data", "players_wl"))
...
def etl_players():
    ...
    players_info_flag, players_info_value = json_to_dataframe(
        f"{PLAYERS_INFO_PATH}\*.json",
        players_info_columns,
        "Extracting players info...",
    )
    # print(players_info_value)
    if players_info_flag == True:
        df_players_info_draw = players_info_value
    else:
    ...
    players_wl_flag, players_wl_value = json_to_dataframe(
        f"{PLAYERS_WL_PATH}/*.json",
        players_info_columns,
        "Extracting players win lose...",
    )
    if players_wl_flag == True:
        df_players_wl = players_wl_value
    else:
        change_multi_rows_values_downloaded(
            str(Path(ROOT, "check_download_player_id.parquet")),
            players_info_value,
    ...
    # df_players_wl = pd.read_csv("lake/player_wl.csv", encoding="utf8")
    ...
    # load
    df_player.to_parquet(OUTPUT_PATH, index=False)
