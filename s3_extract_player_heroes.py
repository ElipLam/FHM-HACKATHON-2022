...
df_check.to_csv("check_download_player_heroes.csv", index=False)

# read list objects from check download file
df_read_check_download = pd.read_csv("check_download_player_heroes.csv")
list_keys = df_read_check_download["key"].tolist()
# df_read_check.to_csv("check_download_player_heroes.csv", index=False)

if Path("new_lake/player_heros.csv").is_file():
    global_df = pd.read_csv("new_lake/player_heroes.csv")
else:
...
try:
    ...
    df_read_check_download.to_csv("check_download_player_heroes.csv", index=False)
    result_df = pd.concat(list_df)
    result_df.to_csv("new_lake/player_heroes.csv", index=False)
